...
# Condition assignment tracking file
ASSIGNMENT_TRACKER_FILE = os.path.join(DATA_DIR, "condition_assignments.csv")
...
def _generate_analysis_if_needed(participant_id, phase):
    """Generate analysis report automatically after certain phases complete"""
    # Only generate analysis after manipulation_check (experiment complete)
    if phase != "manipulation_check":
        return
...
def log_data(participant_id, phase, data):
    ...
    if name:
        ...
        # Determine condition suffix
        if structure == "integrated":
            condition_suffix = "Integrated"
        elif structure == "segmented":
            condition_suffix = "Segmented"
        elif structure:
            condition_suffix = structure.title()
        else:
            # Structure not set yet (e.g., during demographics), use participant_id only for now
            # Will be renamed after randomization
...
def get_localized_article(article_key: str) -> dict:
    ...
    For Chinese: Only uses manual translations from cache (no auto-translation).
    ...
    # For non-English: check cache for manual translations only (no auto-translate)
    localized = {}
    for k in [
        "title",
        "free_recall_prompt",
        ...
        # Check cache for manual translation
...
7. Early prototypes of SHERLOCK and DETECTR were initially explored as tools to monitor DNA repair activity inside cells before shifting to pathogen detection.

8. Delivery remains the major barrier: viral vectors are efficient but small; lipid nanoparticles carry more but risk inflammation.

9. Self-limiting and inducible CRISPR systems control activity duration, improving safety.

10. Germ-line editing is ethically restricted because changes are heritable and affect future generations.''',
        'questions': [
            {
...
@app.route("/")
def index():
    # First check: condition selection (before everything else)
    if "selected_structure_condition" not in session:
        counts = get_condition_counts()
        return render_template("select_condition.html", counts=counts, error=None)
...
def submit_ai_trust():
    ...
    # Save all individual answers, not just the means
    log_data(session["participant_id"], "ai_trust", {
        "ai_trust_score": ai_trust_score,
        "ai_trust_individual": json.dumps(data.get("trust", {})),  # All individual trust question answers
        "ai_dependence_score": ai_dependence_score,
        "ai_dependence_individual": json.dumps(data.get("dependence", {})),  # All individual dependence question answers
        "tech_skill_score": tech_skill_score,
        "tech_skill_individual": json.dumps(data.get("skill", {})),  # All individual skill question answers
...
def get_condition_counts():
    """
    Get current count of participants assigned to each structure condition.
    Returns dict with counts for 'A1_Integrated' and 'A2_Segmented'.
    """
    counts = {'A1_Integrated': 0, 'A2_Segmented': 0}
    
    if os.path.exists(ASSIGNMENT_TRACKER_FILE):
        try:
            with open(ASSIGNMENT_TRACKER_FILE, 'r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    condition = row.get('structureCondition', '')
                    if condition in counts:
                        counts[condition] += 1
        except Exception as e:
            print(f"Error reading assignment tracker: {e}")
    
    return counts

def assign_participant_conditions(participant_id, structure_condition=None):
    """
    Assign counterbalanced conditions to a participant.
    If structure_condition is provided, use it; otherwise use session value or random.
    
    Args:
        participant_id: Participant ID
        structure_condition: Optional - 'A1_Integrated' or 'A2_Segmented'. If None, uses session value.
    
    Returns:
        dict with structureCondition, timingOrder, article1Timing, article2Timing, article3Timing
    """
    # Step 1: Get structure condition from parameter, session, or random
    if structure_condition is None:
        structure_condition = session.get('selected_structure_condition')
    ...
    # Step 4: Save assignment to tracker file
    file_exists = os.path.exists(ASSIGNMENT_TRACKER_FILE)
    try:
        with open(ASSIGNMENT_TRACKER_FILE, 'a', encoding='utf-8', newline='') as f:
            fieldnames = ['participantId', 'structureCondition', 'timingOrder', 
                         'article1Timing', 'article2Timing', 'article3Timing', 'assignmentTimestamp']
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            if not file_exists:
                writer.writeheader()
            writer.writerow(assignment)
    except Exception as e:
        print(f"Error writing to assignment tracker: {e}")
    
    return assignment

@app.route("/select_condition", methods=["GET", "POST"])
def select_condition():
    """
    Page to select AI summary condition (integrated paragraph vs bullet points).
    Shown BEFORE language selection - no participant_id required yet.
    """
    if request.method == "POST":
        selected = request.form.get("structure_condition")
        if selected in STRUCTURE_CONDITIONS:
            session["selected_structure_condition"] = selected
            session.modified = True
            # Log condition selection (participant_id may not exist yet)
            if "participant_id" in session:
                log_data(session["participant_id"], "condition_selection", {
                    "selected_condition": selected,
                    "timestamp": datetime.now().isoformat()
                })
            # Redirect to language selection (next step)
            return redirect(url_for("index"))
        else:
            counts = get_condition_counts()
            return render_template("select_condition.html", error="Please select a valid condition.", counts=counts)
...
def randomize():
    ...
    # Check if condition is selected, if not redirect to selection
    if 'selected_structure_condition' not in session:
        return redirect(url_for('select_condition'))
    
    # Assign counterbalanced conditions using the selected structure condition
    participant_id = session["participant_id"]
    assignment = assign_participant_conditions(participant_id, session["selected_structure_condition"])
...
def test_phase(article_num: int):
    ...
    # Use questions in original order (no randomization)
    ...
    return render_template(
        ...
        article_key=article_key,
        article_title=article["title"],
        questions=questions_list,  # Use original order
        test_mode=TEST_MODE,
...
def submit_test():
    ...
    if mcq_data:
        ...
        # Questions are no longer randomized - use direct index mapping
        # Guard: require all MCQ questions answered before proceeding
        if article and "questions" in article:
            questions = article["questions"]
            missing_or_invalid = []
            # Check using direct indices (no randomization)
        ...
        if article and "questions" in article:
            questions = article["questions"]
            # Process answers using direct indices (no randomization)
            for q_idx in range(len(questions)):
                total_questions += 1
                q_key = f"q{q_idx}"  # Answer key from frontend
                participant_answer = mcq_data.get(q_key, None)
                question = questions[q_idx] if q_idx < len(questions) else None
                
                # Debug logging
                print(f"[MCQ DEBUG] Processing q{q_idx}: "
        ...
        log_data(session["participant_id"], "mcq_responses", {
            ...
            "question_mapping": json.dumps({})  # No randomization - empty mapping
        })
