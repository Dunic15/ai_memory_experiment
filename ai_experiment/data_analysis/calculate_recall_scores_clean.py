...
# Data extracted from the user-provided data
...
def calculate_averages():
    ...
    print(f"  Pre-reading:   {avg(integrated_pre):.2f} (n={len(integrated_pre)})")
    print(f"  Synchronous:   {avg(integrated_sync):.2f} (n={len(integrated_sync)})")
    print(f"  Post-reading:  {avg(integrated_post):.2f} (n={len(integrated_post)})")
    print(f"  → Average for Integrated: {avg(integrated_pre + integrated_sync + integrated_post):.2f}")
    print()
    
    print("SEGMENTED (With AI):")
    print(f"  Pre-reading:   {avg(segmented_pre):.2f} (n={len(segmented_pre)})")
    print(f"  Synchronous:   {avg(segmented_sync):.2f} (n={len(segmented_sync)})")
    print(f"  Post-reading:  {avg(segmented_post):.2f} (n={len(segmented_post)})")
