...
try:
    ...
    from app_control import ARTICLES
except ImportError:
    ARTICLES = {}
...
def parse_csv_log(log_file_path):
    ...
    # CONTROL VERSION: Removed AI-related fields
    data = {
        'demographics': {},
        'prior_knowledge': {},
        'randomization': {},
    ...
    with open(log_file_path, 'r', encoding='utf-8', newline='') as f:
        ...
        for parts in reader:
            ...
            try:
                ...
                # CONTROL VERSION: ai_trust phase removed
                elif phase == 'randomization':
                    # CONTROL VERSION: No structure or timing_order
                    data['randomization'] = {
                        'article_order': parts[2] if len(parts) > 2 else '',
                        'timestamp': timestamp
                    }
                    ...
                    if event_type == 'reading_complete':
                        # CONTROL VERSION: Simplified schema - no summary or timing
                        # Schema: timestamp, phase, reading_complete, timestamp2, reading_time_ms, scroll_depth, article_num, article_key
                        reading_time_ms = 0
                        ...
                        # Extract scroll depth (index 5)
                        if len(parts) > 5:
                            try:
                                scroll_depth = int(parts[5])
                            except:
                                pass
                        
                        # Extract article number (index 6)
                        if len(parts) > 6:
                            try:
                                article_num = int(parts[6])
                            except:
                                pass
                        
                        # Extract article_key (index 7)
                        if len(parts) > 7:
                            article_key = parts[7]
                    ...
                    # CONTROL VERSION: summary overlay events removed
                    elif False and event_type == 'summary_overlay_opened':
                    ...
                    # Handle visibility changes
                        ...
                        # CONTROL VERSION: Simplified - no timing
                        # Schema: timestamp, phase, visibility_change, timestamp2, is_visible, article_num, article_key
                        vis_article_num = -1
                        is_visible = False
                        vis_article_key = ''
                        if len(parts) > 4:
                ...
                # CONTROL VERSION: summary_viewing phase removed
                elif False and phase == 'summary_viewing':
                    if len(parts) >= 8:
                    ...
                    # CONTROL VERSION: Simplified - no timing
                    # csv.reader already joined multiline recall text into a single field (index 4)
                    if len(parts) >= 12:
                        article_num = int(parts[2]) if parts[2] else -1
                        article_key = parts[3]
                        recall_text = parts[4] if len(parts) > 4 else ''
                        ...
                        data['recall_data'].append({
                            ...
                            'sentence_count': _as_int(5),
                            'word_count': _as_int(6),
                            'character_count': _as_int(7),
                            'confidence': _as_int(8),
                            'difficulty': _as_int(9),
                            'time_spent_ms': _as_int(10),
                            'paste_attempts': _as_int(11),
                            'over_limit': (parts[12].lower() == 'true') if len(parts) > 12 else False
                        })
                elif phase == 'mcq_responses':
                    # CONTROL VERSION: Simplified - no timing
                    # Expected columns:
                    # timestamp, phase, article_num, article_key, answers_json, answer_times_json, total_time_ms, correct_count, total_questions, accuracy_rate, question_accuracy_json
                    if len(parts) >= 5:
                        answers_json = parts[4]
                        try:
                    ...
                    # Schema: timestamp, phase, article_num, article_key,
                    # load_mental_effort, load_task_difficulty, mcq_overall_confidence
                    if len(parts) >= 6:
                        ...
                        data['post_article_ratings'].append({
                            ...
                            'article_num': article_num,
                            'article_key': article_key,
                            'load_mental_effort': _as_int_safe(4),
                            'load_task_difficulty': _as_int_safe(5),
                            'mcq_overall_confidence': _as_int_safe(6)
                        })
                # CONTROL VERSION: manipulation_check removed
                elif False and phase == 'manipulation_check':
                    if len(parts) >= 4:
...
def calculate_mcq_accuracy(mcq_data):
    ...
    for mcq in mcq_data:
        ...
        results.append({
            ...
            'timing': mcq.get('timing', 'control'),  # Default to 'control' for no_ai experiment
            'accuracy': accuracy,
...
def generate_analysis_report(participant_id, data, mcq_results):
    ...
    if data.get('randomization'):
        ...
        structure = rand.get('structure', 'CONTROL').upper()  # Default to CONTROL for no_ai
        structure_text = structure
        if structure == 'SEGMENTED':
            structure_text += " (bullet points)"
        elif structure == 'CONTROL' or structure == 'N/A':
            structure_text = "CONTROL (No AI summaries)"
    ...
    reading_by_article = {}
    for rd in data.get('reading_data', []):
    ...
    for sv in data.get('summary_viewing', []):
    ...
    sync_summaries = {}
    for event in data.get('summary_overlay_events', []):
    ...
    sync_articles = [rd for rd in data.get('reading_data', []) if rd.get('timing') == 'synchronous']
    ...
    recall_by_article = {}
    for rec in data.get('recall_data', []):
    ...
    ratings_by_article = {}
    for rating in data.get('post_article_ratings', []):
    ...
    for (article_key, article_num), ratings in ratings_by_article.items():
        ...
        rating = ratings[0]
        article_name = article_names.get(article_key.lower() if article_key else '', article_key.upper() if article_key else 'Unknown')
        # CONTROL VERSION: No timing in control version
        
        report.append(f"Article {article_num + 1 if article_num >= 0 else '?'} ({article_name}):")
        report.append("")
    ...
    if not data.get('post_article_ratings'):
    ...
    report.append("=" * 80)
    if data.get('manipulation_check'):
    ...
    if data.get('reading_data'):
        reading_times = [rd['reading_time_ms'] / 60000 for rd in data.get('reading_data', [])]
    ...
    if data.get('summary_viewing') or sync_summaries:
        report.append("")
        ...
        all_summary_times = [sv['time_spent_seconds'] / 60 for sv in data.get('summary_viewing', [])]
        ...
        all_summary_times.extend([rd.get('summary_time_ms', 0) / 60000 for rd in data.get('reading_data', []) 
                                 if rd.get('timing') == 'synchronous' and rd.get('summary_time_ms', 0) > 0 
    ...
    if data.get('recall_data'):
        report.append("")
        report.append("Recall Quality:")
        recall_data = data.get('recall_data', [])
        avg_words = sum(rec['word_count'] for rec in recall_data) / max(1, len(recall_data))
        avg_sentences = sum(rec['sentence_count'] for rec in recall_data) / max(1, len(recall_data))
        avg_chars = sum(rec['character_count'] for rec in recall_data) / max(1, len(recall_data))
        avg_confidence = sum(rec['confidence'] for rec in recall_data) / max(1, len(recall_data))
        avg_difficulty = sum(rec['difficulty'] for rec in recall_data) / max(1, len(recall_data))
    ...
    if data.get('reading_data'):
        ...
        scroll_depths = [rd.get('scroll_depth', 100) for rd in data.get('reading_data', [])]
    ...
    if data.get('summary_viewing'):
        findings.append("")
        findings.append("2. SUMMARY VIEWING:")
        pre_reading = [sv for sv in data.get('summary_viewing', []) if sv.get('mode') == 'pre_reading']
        post_reading = [sv for sv in data.get('summary_viewing', []) if sv.get('mode') == 'post_reading']
    ...
    if data.get('recall_data'):
        ...
        recall_data = data.get('recall_data', [])
        word_counts = [rec['word_count'] for rec in recall_data]
        sentence_counts = [rec['sentence_count'] for rec in recall_data]
        confidences = [rec['confidence'] for rec in recall_data]
        difficulties = [rec['difficulty'] for rec in recall_data]
        ...
        recall_data = data.get('recall_data', [])
        paste_attempts = [rec.get('paste_attempts', 0) for rec in recall_data]
        for i, rec in enumerate(recall_data):
    ...
    if all(rd.get('scroll_depth', 0) >= 95 for rd in data.get('reading_data', [])):
        strengths.append("  - High engagement (95%+ scroll depth)")
    
    if len(data.get('recall_data', [])) >= 3 and len(mcq_results) >= 3:
...
def main():
    ...
    mcq_results = calculate_mcq_accuracy(data.get('mcq_data', []))
