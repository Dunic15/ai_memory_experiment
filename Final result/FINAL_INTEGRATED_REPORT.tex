...
\subsubsection{Mechanism: The Advance Organizer Effect}

Why does pre-reading work? The summary functions as an \textbf{advance organizer} \citep{ausubel1960}:
\begin{itemize}
    \item Activates relevant schemas before encoding
    \item Provides a ``cognitive map'' that guides attention during reading
    \item Reduces extraneous cognitive load
\end{itemize}
...
\begin{tabular}{lcc}
\toprule
\textbf{Timing} & \textbf{AI Summary Accuracy} & \textbf{Summary Time (sec)} \\
\midrule
\textbf{Pre-reading} & \textbf{0.833} & \textbf{132.5} \\
Synchronous & 0.568 & 100.3 \\
Post-reading & 0.641 & 69.5 \\
\bottomrule
\end{tabular}
\caption{Process Variables by Timing Condition}
\end{table}
...
\subsubsection{Mechanism: Source Monitoring Failure}

Why do segmented summaries increase false memories?

\begin{enumerate}
    \item \textbf{Fragmented mental representation:} Segmented layouts make it harder to build a coherent mental model
    \item \textbf{Reduced cross-checking:} Separated information discourages verification
    \item \textbf{Automation bias:} Segmentation may encourage passive acceptance
    \item \textbf{Misinformation effect:} Particularly strong in post-reading
\end{enumerate}

\textbf{Theoretical grounding:}
\begin{itemize}
    \item Source Monitoring Framework \citep{johnson1993}
    \item Cognitive Load Theory---split attention effect \citep{chandler1992}
    \item Misinformation effect literature \citep{loftus2005}
\end{itemize}
...
\subsubsection{Mechanism: Different Memory Processes}

This dissociation reflects a fundamental distinction in how memory operates:

\begin{enumerate}
    \item \textbf{MCQ = cue-supported recognition:} When answer options are presented, the learner only needs to recognize which option ``feels right.'' The summary's organizational framework activates relevant schemas, making correct answers more familiar and easier to identify. This is a \textit{passive} retrieval process---the answer is provided, and the learner evaluates it.
    
    \item \textbf{Recall = self-generated retrieval:} Free recall demands that learners actively reconstruct information from memory without external cues. The summary provides a \textit{macrostructure} (main ideas, organization) but does not strengthen the \textit{microstructure} (specific details, verbatim traces) needed for recall.
\end{enumerate}

\textbf{Why this matters:} This finding has important implications for educational assessment. If an instructor uses AI summaries and only tests with MCQs, they may overestimate how much students actually learned. The benefit of pre-reading AI summaries is \textit{task-specific}---it enhances recognition but not generative retrieval.

\textbf{Practical implication:} To improve recall, learners need \textit{active generation} during study (e.g., self-testing, elaborative interrogation), not just organized exposure to content.
...
This pattern aligns with Tulving's \textbf{Encoding Specificity Principle} \citep{sparrow2011}:
\begin{quote}
...
\textbf{Related Theory: Transactive Memory (Wegner, 1987)}\\[0.3cm]
The AI summary may function as an \textbf{external memory system}, where learners encode \textit{where} information is stored (the AI) rather than the information itself---a phenomenon increasingly relevant in the digital age (``Google effects on memory'').
...
\textbf{Automation Bias} \citep{risko2016}: Users may over-rely on automated systems, accepting their outputs without verification. The segmented format may exacerbate this by making verification more cognitively costly (split-attention effect).
...
\textbf{Cognitive Offloading:} Segmented format may encourage treating AI as an external memory store (offloading) rather than as a tool for active learning---leading to dependence without trust.
...
\subsection{Two Independent Cognitive Pathways}

The data reveal two separable causal pathways:

\begin{tcolorbox}[colback=green!5!white,colframe=findinggreen,title=Pathway 1: Timing $\rightarrow$ Encoding Quality $\rightarrow$ MCQ]
Pre-reading $\rightarrow$ Better schema activation + summary accuracy $\rightarrow$ Higher MCQ accuracy ($d = 1.35+$)
\end{tcolorbox}

\begin{tcolorbox}[colback=orange!5!white,colframe=warningorange,title=Pathway 2: Structure $\rightarrow$ Representation Coherence $\rightarrow$ False Lures]
Segmented $\rightarrow$ Fragmented mental model + source confusion $\rightarrow$ More false lure errors (OR $\approx 6\times$)
\end{tcolorbox}

\textbf{Critical insight:} These pathways are largely \textbf{independent}. You can maximize MCQ performance with pre-reading AND minimize false memories with integrated format.
...
The pre-reading advantage aligns with \textbf{Ausubel's Advance Organizer Theory} \citep{ausubel1960}:
...
\begin{enumerate}
    ...
    \item Facilitates meaningful learning over rote memorization
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Extraneous load:} Reduced because learners don't need to ``figure out what matters''
...
The structure effect aligns with the \textbf{Source Monitoring Framework} \citep{johnson1993}:
...
\begin{enumerate}
    ...
    \item Spatial separation increases misattribution risk
...
\textbf{Fuzzy-Trace Theory} \citep{brainerd2002} explains the mechanism:
\begin{itemize}
...
The dissociation aligns with dual-process theories \citep{koriat1997}:
\begin{itemize}
    \item \textbf{MCQ (recognition):} Solved via familiarity-based processes
    \item \textbf{Recall:} Requires recollection-based retrieval
\end{itemize}

\subsubsection{Generation Effect}
...
Free recall requires \textbf{active generation}, not just recognition. Pre-reading provides passive exposure that helps recognize answers but doesn't create generative retrieval pathways.
...
\begin{itemize}[noitemsep]
    ...
    \item Jacoby (1991)
    \item Morris, Bransford, \& Franks (1977)
...
\textbf{Timing $\neq$ Recall} & Roediger \& Karpicke (2006); Jacoby (1991); Morris et al. (1977) & \checkmark~MCQ-recall dissociation is well-documented in dual-process and transfer-appropriate processing literature \\
