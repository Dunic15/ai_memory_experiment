#!/usr/bin/env python3
"""
Clean duplicate entries and analyze new participants for specific metrics:
- Total MCQ %
- AI Summary %
...
def parse_log_file_clean(log_file_path, is_ai_experiment=True):
    """Parse log file and extract MCQ data, removing duplicates."""
    mcq_data = {}  # Use dict to track unique article entries
    participant_name = ""
    duplicates_found = 0
    ...
    with open(log_file_path, 'r', encoding='utf-8', newline='') as f:
        ...
        try:
            ...
            return [], participant_name, duplicates_found
        ...
        for parts in reader:
            ...
            if phase == 'mcq_responses':
                ...
                # Check if we already have this article (duplicate detection)
                if article_key in mcq_data:
                    duplicates_found += 1
                    continue  # Skip duplicate
                
                mcq_data[article_key] = {
                    'article_key': article_key,
                    'question_details': question_details
                }
    
    return list(mcq_data.values()), participant_name, duplicates_found

def analyze_participant(log_file_path, is_ai_experiment=True):
    """Analyze a participant's MCQ performance by source type."""
    mcq_data, participant_name, duplicates = parse_log_file_clean(log_file_path, is_ai_experiment)
...
def main():
    ...
    print("=" * 110)
    print("AI EXPERIMENT - NEW PARTICIPANTS ANALYSIS (CLEANED - Duplicates Removed)")
    print("=" * 110)
    print()
    print(f"{'Participant':<12} {'Total MCQ':<10} {'AI Summary':<12} {'False Lure':<12} {'FL Selected':<12} {'Article':<10} {'Duplicates':<10}")
    print("-" * 110)
    ...
    for pid in ai_participants:
        ...
        if log_file:
            ...
            dup_note = f"{result['duplicates_found']} removed" if result['duplicates_found'] > 0 else "0"
            print(f"{pid:<12} {result['total_pct']:>8.1f}% {result['ai_summary_pct']:>10.1f}% {result['false_lure_pct']:>10.1f}% {result['false_lures_selected']:>10} {result['article_pct']:>8.1f}% {dup_note:>10}")
        else:
            print(f"{pid:<12} {'NOT FOUND':<10}")
    ...
    if ai_results:
        ...
        total_dups = sum(r['duplicates_found'] for r in ai_results)
        print("-" * 110)
        print(f"{'AVERAGE':<12} {avg_total:>8.1f}% {avg_ai:>10.1f}% {avg_fl:>10.1f}% {avg_fl_sel:>10.1f} {avg_art:>8.1f}% {total_dups:>10}")
    
    print()
    print("=" * 110)
    print("NO-AI EXPERIMENT - NEW PARTICIPANTS ANALYSIS (CLEANED)")
    print("=" * 110)
    print()
    print(f"{'Participant':<12} {'Total MCQ':<10} {'AI Summary':<12} {'False Lure':<12} {'FL Selected':<12} {'Article':<10} {'Duplicates':<10}")
    print("-" * 110)
    ...
    for pid in no_ai_participants:
        ...
        if log_file:
            ...
            dup_note = f"{result['duplicates_found']} removed" if result['duplicates_found'] > 0 else "0"
            print(f"{pid:<12} {result['total_pct']:>8.1f}% {'N/A':>10} {'N/A':>10} {'N/A':>10} {result['article_pct']:>8.1f}% {dup_note:>10}")
        else:
            print(f"{pid:<12} {'NOT FOUND':<10}")
    ...
    if no_ai_results:
        ...
        total_dups = sum(r['duplicates_found'] for r in no_ai_results)
        print("-" * 110)
        print(f"{'AVERAGE':<12} {avg_total:>8.1f}% {'N/A':>10} {'N/A':>10} {'N/A':>10} {avg_art:>8.1f}% {total_dups:>10}")
    
    # Validation: Check false lure logic
    print()
    print("=" * 110)
    print("VALIDATION: False Lure Analysis Detail")
    print("=" * 110)
    print()
    print("For AI experiment, each article has 2 false lure questions (6 total per participant).")
    print("'FL Selected' = times participant chose the FALSE LURE option (misinformation from AI)")
    print("'FL %' = accuracy on false lure questions (can get wrong via other wrong answers too)")
    print()
    print(f"{'PID':<8} {'FL Qs':<8} {'FL Correct':<12} {'FL Wrong':<10} {'FL Selected':<12} {'FL %':<8}")
    print("-" * 60)
    for result in ai_results:
        fl_total = result['false_lure_total']
        fl_correct = round(result['false_lure_pct'] * fl_total / 100) if fl_total > 0 else 0
        fl_wrong = fl_total - fl_correct
        fl_selected = result['false_lures_selected']
        
        # Consistency check: FL Selected should be <= FL Wrong
        status = "✓" if fl_selected <= fl_wrong else "✗ ERROR"
        print(f"{result['pid']:<8} {fl_total:<8} {fl_correct:<12} {fl_wrong:<10} {fl_selected:<12} {result['false_lure_pct']:.1f}% {status}")
