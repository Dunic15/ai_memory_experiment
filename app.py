...
def _auto_translate(text: str, target_lang: str) -> str:
    """
    Placeholder for step 2: automatic translation.
    For now, return original if English or translator unavailable.
    """
    ...
    try:
        if GoogleTranslator is None:
            return text
        # We'll wire this in step 2; keeping behavior safe for now.
...
ARTICLES = {
    'uhi': {
        ...
        'free_recall_prompt': 'Please write everything you remember from the article within 3 minutes. Try to describe main ideas and relationships â€” causes, consequences, and solutions â€” in your own words.',
        'text': '''Cities are artificial landscapes that capture sunlight and store heat. Every day, roofs, roads, and facades absorb radiant energy from the sun, converting it into thermal mass that lingers long after sunset. Unlike forests or grasslands, which reflect part of the incoming radiation and cool themselves through evapotranspiration, the dense materials of citiesâ€”concrete, asphalt, brick, and metalâ€”accumulate heat during daylight and release it slowly through the night. This process generates a persistent temperature difference between urban and rural zones known as the urban heat island effect (UHI). The phenomenon is not limited to hot afternoons: it reshapes the entire twenty-four-hour temperature cycle, raising minimum nighttime values and extending periods of discomfort for residents. In many cities, nights can remain three to seven degrees Celsius warmer than surrounding countryside, altering sleep quality, increasing electricity demand, and intensifying health risks during heat waves.
...
Cities that pursue integrated strategiesâ€”combining reflection, shading, ventilation, and water managementâ€”demonstrate that progress is achievable even in warming climates. Tokyo, for example, has implemented widespread reflective surfaces and mandatory greening of public facilities, observing measurable nighttime temperature reductions. Singapore integrates vertical greenery and sky gardens into high-rise architecture, creating layered shade that cools multiple levels simultaneously. In Europe, Parisâ€™s post-2003 heat-wave reforms have led to cool-roof standards and the re-vegetation of schoolyards as emergency shelters during extreme events. These examples show that urban heat islands are not inevitable by-products of modernization but reversible artifacts of design.
...
- Urban nights stay 3â€“7 Â°C warmer, raising electricity demand and health risks.
...
- Cool roofs cut rooftop temperatures 20â€“30 Â°C and lower indoor cooling 10â€“20 %.
- Equity-based programs prioritize public buildings and tree-poor districts.''',
        'questions': [
            ...
            {"q": "Typical rooftop temperature drop from cool roofs is about ________.", "options": ["5â€“10 Â°C", "20â€“30 Â°C", "35â€“45 Â°C", "None"], "correct": 1},
            {"q": "Energy savings from cool roofs are roughly ________ of indoor cooling loads.", "options": ["1â€“5 %", "5â€“10 %", "10â€“20 %", "30â€“40 %"], "correct": 2},
            ...
            {"q": "At night, urban temperatures can remain about ________ warmer than rural areas.", "options": ["1â€“2 Â°C", "3â€“7 Â°C", "8â€“10 Â°C", "> 10 Â°C"], "correct": 1},
    ...
    'crispr': {
        ...
        'text': '''CRISPRâ€“Cas systems began as a microbial defense mechanismâ€”a molecular immune memory that bacteria use to recognize and destroy invading viruses. Each infection leaves a short fragment of viral DNA inserted into the bacterial genome, serving as a permanent record of attack. When the same virus returns, the bacteria transcribe these fragments into RNA guides that lead Cas enzymes to the matching sequence, cutting it apart. This elegant process of recognition and cleavage inspired scientists to reprogram it for their own purposes. By designing a synthetic guide RNA that matches any chosen DNA region, researchers can direct the Cas enzyme to that exact spot, slice the double helix, and let the cellâ€™s repair machinery rewrite it. This simple principleâ€”guide, cut, and repairâ€”has turned a bacterial survival trick into one of the most versatile tools in modern science.
The methodâ€™s accessibility has been revolutionary. What once required months of effort with older tools such as zinc-finger nucleases or TALENs can now be done in days, using inexpensive reagents that fit in any basic lab. The democratization of gene editing has allowed teams across the world to explore genetic diseases, agricultural improvement, and environmental restoration at unprecedented speed. Yet this same simplicity conceals layers of complexity. The word precision suggests perfection, but in genomics, precision is statistical. When the guide RNA pairs exactly with its target, Cas cuts accurately, but genomes are enormous and filled with repetitive sequences. Slight mismatches can cause unintended cuts at similar sites, producing off-target edits that may disrupt other genes.
...
Ultimately, CRISPR is not merely a technical invention but a mirror of collective values. It reveals how societies balance innovation with caution, competition with solidarity, and individual ambition with public good. When data are shared transparently, benefits distributed equitably, and oversight continuous, gene editing can transform from a disruptive novelty into a stable element of medicine, agriculture, and conservation. The true measure of its success will not be the number of genomes edited, but the fairness and foresight with which those edits are made. CRISPRâ€™s legacy will be written not only in DNA sequences but in the choices humanity makes about whose future is worth rewriting
''',
        'summary_integrated': 'CRISPRâ€“Cas technology re-purposes a bacterial immune system into a programmable gene-editing tool. A guide RNA directs the Cas enzyme to a chosen DNA sequence, where cellular repair can delete, insert, or correct genetic material. Its appeal lies in speed and low cost, yet precision is layered: off-target edits may arise when similar genomic regions are cut unintentionally. Researchers now refine guide design, engineer high-fidelity enzymes, and employ base or prime editors that swap letters without double-strand breaks. Delivery remains the toughest barrierâ€”viral vectors, lipid nanoparticles, and new biodegradable carriers must reach the right tissues without provoking immunity. In medicine, ex vivo editing corrects blood-cell disorders under laboratory control, while in vivo applications for organs such as brain or heart demand higher safety. Germ-line editing of embryos remains largely prohibited because risks transmit to future generations. Agriculture uses CRISPR to strengthen crops and reduce pesticide use, requiring transparent regulation and public trust. Ethical oversight must balance innovation with equity through inclusive review boards, data openness, and long-term monitoring. Emerging editors, self-limiting delivery systems, and global governance frameworks will shape the next decade. CRISPRâ€™s success will be judged less by technical speed than by fairness, safety, and the integrity with which societies choose what to edit.',
        'summary_segmented': '''- CRISPRâ€“Cas re-purposes bacterial immunity to cut and repair specific DNA sequences.
...
''',
        'questions': [
            ...
            {"q": "To prevent â€œgenomic monopolies,â€ global frameworks should ensure ______.", "options": ["Open licensing and benefit-sharing", "Private ownership", "Patent secrecy", "National exclusivity"], "correct": 0}
        ]
    },
    'semiconductors': {
...
Flexibility complements visibility. Designs that accept functionally equivalent componentsâ€”known as second sourcingâ€”enable substitution when a vendor falters. Engineers now favor circuit boards that can accommodate multiple pin-compatible microcontrollers or memory modules. Software abstraction layers allow firmware to run across chips from different brands. These design practices reduce dependency on single suppliers without sacrificing performance.
Contracts form the third lever. Traditional purchasing treated chips like commodities: the lowest bidder won. During shortages, that logic collapses because supply allocation favors relationships, not price. Some automakers now sign take-or-pay contracts that guarantee foundries a minimum revenue even if volumes fluctuate. Others pre-pay for capacity, effectively sharing risk. The aim is alignment: when customers signal long-term commitment, producers invest confidently. Governments can support this shift through co-investment schemes or tax incentives that tie subsidies to open accessâ€”ensuring that new fabs serve multiple industries rather than a single corporate patron.
Communication and data sharing close the loop. Rolling twelve-month forecasts and early-warning dashboards let upstream partners plan equipment maintenance and adjust production mix. In complex supply networks, signal quality is a public good: the clearer the information, the smaller the panic when demand shifts. Many governments are funding â€œsemiconductor observatoriesâ€ to collect anonymized order data and provide aggregated market indicators. The goal is not central planning but transparency, reducing the amplitude of collective over-reaction.
The semiconductor shortage also exposed cultural habits of over-optimization. For decades, supply-chain excellence meant cutting every marginâ€”inventory, redundancy, and idle capacity were treated as waste. That mindset maximized short-term efficiency but sacrificed resilience. Economists now argue for a new balance: an efficiencyâ€“resilience frontier rather than a single optimum. On one side lies lean precision; on the other, slack capacity that absorbs shocks. Moving along this curve is strategic, not wasteful. Just as financial portfolios hedge against volatility, industrial portfolios hedge against supply disruption.
...
- Trust-based partnerships with transparent data enable shock absorption and faster recovery.''',
        'questions': [
            {'q': 'Why did automakers halt production during 2020â€“2022?', 'options': ['Lack of high-end GPUs', 'Shortage of low-cost microcontrollers', 'Labor strikes', 'Currency crisis'], 'correct': 1},
...
PRIOR_KNOWLEDGE_TERMS = [
    "Albedo", "Evapotranspiration", "CRISPR", "Guide RNA",
    "Off-target editing", "Foundry", "Photolithography",
    "Microcontroller", "Thermal mass", "Street canyon"
]

PRIOR_KNOWLEDGE_QUIZ = [
    {"q": "What does *albedo* measure?", "options": ["Heat capacity", "Reflectivity", "Humidity", "Wind speed"], "correct": 1},
    {"q": "*CRISPR* is primarily a tool for:", "options": ["Protein folding", "Genome editing", "MRI imaging", "Battery storage"], "correct": 1},
    {"q": "A *microcontroller* is best described as:", "options": ["High-end GPU", "Small integrated computer on a chip", "Wireless antenna", "Power converter"], "correct": 1},
    {"q": "*Photolithography* is used to:", "options": ["Package chips", "Pattern circuits on wafers", "Ship containers", "Mine silicon"], "correct": 1},
    {"q": "*Evapotranspiration* refers to:", "options": ["Heat conduction through stone", "Water loss from soil and plants", "Wind mixing in canyons", "Solar glare"], "correct": 1},
]
...
# ------------------------------------------------------------------------------
@app.route("/recall_instruction/<int:i>")
@require_pid
...
def skip_reading():
    ...
    # Skip current reading â†’ go to the 5â€‘minute preâ€‘test break for this article
...
@require_pid
def prior_knowledge():
    # Start (or reuse) a fixed 5â€‘minute window for this section
    ...
    return render_template(
        ...
        terms=PRIOR_KNOWLEDGE_TERMS,
        quiz=PRIOR_KNOWLEDGE_QUIZ,
...
def submit_prior_knowledge():
    ...
    # Reset PK timer so revisits (if any) start a fresh 5â€‘minute window
...
def submit_ai_trust():
    ...
    # Reset AIâ€‘Trust timer so revisits (if any) start a fresh 5â€‘minute window
...
def reading_phase(article_num: int):
    ...
    session["current_timing"] = timing_order[article_num]
    session["reading_start_time"] = datetime.now().isoformat()

    # Summary window: allow viewing the AI summary for at most 90 seconds from first load
    if "summary_started_at" not in session or session.get("current_article") != article_num:
        session["summary_started_at"] = datetime.now().timestamp()
    summary_deadline = session["summary_started_at"] + 90  # seconds

    # Summary lock: once a participant leaves the summary to start reading,
    # they cannot open the summary again for this article.
    summary_locked = bool(session.get(f"summary_locked_{article_num}", False))
    if summary_locked:
        summary_deadline = 0
...
@app.route("/dev/recall_instruction_bypass/<int:article_num>")
@require_pid
